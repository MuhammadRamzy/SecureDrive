...
def initiate_handshake(device):
    ...
    password = getpass.getpass(
        prompt="[*] Enter SecureDrive Password to unlock vault (or type 'RESET' to reconfigure): "
    )
    ...
    try:
        master_key = cipher_vault.decrypt_and_verify(
            bytes.fromhex(header_data["ciphertext"]), bytes.fromhex(header_data["tag"])
        )
    ...
    try:
        ...
        if mount_result.returncode == 0:
            ...
            print(
                "[*] Open a second terminal and run: sudo mkfs.vfat /mnt/secure_drive/vault.img"
            )
